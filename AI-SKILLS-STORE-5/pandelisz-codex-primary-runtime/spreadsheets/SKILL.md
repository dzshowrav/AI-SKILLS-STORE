...
# Tools + Contract
- Author, edit, inspect, render, and export workbooks only with the JavaScript `@oai/artifact-tool` spreadsheet APIs from the managed Codex runtime.
- Put the executable JavaScript builder under the runtime Node tree so bare ESM `@oai/artifact-tool` imports resolve from the installed runtime `node_modules`:
  ```bash
  set -e
  RUNTIME_ROOT="${CODEX_PRIMARY_RUNTIME:-$HOME/.cache/codex-runtimes/codex-primary-runtime}"
  NODE="$RUNTIME_ROOT/dependencies/node/bin/node"
  SCRIPT_DIR="$RUNTIME_ROOT/dependencies/node/tmp/spreadsheets/<unique_thread_id>"
  OUTPUT_DIR="outputs/<unique_thread_id>"
  SCRIPT="$SCRIPT_DIR/<script>.mjs"
  mkdir -p "$SCRIPT_DIR" "$OUTPUT_DIR"
  # Create or patch "$SCRIPT" directly before running.
  "$NODE" "$SCRIPT"
  cp "$SCRIPT" "$OUTPUT_DIR/$(basename "$SCRIPT")"
  ```
- Run from the user's current working directory. Write final `.xlsx` files, rendered previews, copied builder scripts, and any intermediate data under `outputs/<unique_thread_id>/` with stable, descriptive filenames.
- Final user-facing response must include a short summary of the workbook/sheets/ranges created or edited, plus standalone Markdown link(s) only to final `.xlsx` artifact(s), with link text formatted as `<workbook or primary sheet title> - <filename>` and an absolute filesystem path as the target.
- Use the output-chip-compatible Markdown link shape, for example `[Revenue Model - model.xlsx](/absolute/path/to/model.xlsx)`; replace the example title, filename, and placeholder path with the actual values. If there are multiple requested final workbooks, put each final `.xlsx` Markdown link on its own line. Do not wrap final artifact links in backticks or code fences, and do not put them in bullets, headings, or prose sentences.
...
- Patch and rerun the same temp builder file when iterating. Do not put script bodies in bash heredocs or keep workspace-local builder copies.
...
- For analysis outside workbook authoring, use the simplest reliable installed tool. If analysis cannot be done directly in the JavaScript script or via formulas in the spreadsheet, Python libraries such as `pandas`/`numpy` are available in the bundled Python runtime at `$RUNTIME_ROOT/dependencies/python/bin/python3` (default `~/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3`). Save analysis outputs as JSON/CSV in `OUTPUT_DIR`; the JavaScript builder should read those files and create the workbook with `@oai/artifact-tool`. Keep workbook-derived or user-editable calculations as spreadsheet formulas when auditability or future editing matters.
