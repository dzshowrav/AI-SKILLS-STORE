...
// ❌ Bad
const octokit = new Octokit({ auth: 'ghp_hardcoded_token_here' }); // NEVER DO THIS!
```
...
async function cloneSettings(sourceRepo, targetRepo) {
  const octokit = new Octokit({ auth: process.env.GITHUB_TOKEN });
  const { data: source } = await octokit.repos.get({ owner: 'username', repo: sourceRepo });
  await octokit.repos.update({
    owner: 'username', repo: targetRepo,
    description: source.description, homepage: source.homepage,
    has_issues: source.has_issues, has_wiki: source.has_wiki,
...
async function updateMultipleFiles(owner, repo, files) {
  ...
  for (const file of files) {
    try {
      const url = await updateRepoFile(owner, repo, file.path, file.content, file.message);
