...
## \_inbox と \_knowledge の使い分け

- `_inbox/` は、未整理のチャット、メール、口頭メモ、断片情報を時系列で蓄積する場所。
- `_knowledge/` は、後から再利用できるように抽象化した短い知見、判断基準、Gotcha、検証観点を置く場所。
- 生ログや顧客固有情報を `_knowledge/` に直接移さない。必要なら顧客名、人物名、チケットID、ローカルパス、未公開情報を取り除いて一般化する。
- 「ナレッジ化」「汎用知見を抜き出して」など明示依頼がある場合だけ `_knowledge/` に追記する。
...
| コスト, 費用, 予算, 消費                               | `#cost`         |
| 契約, リニューアル, 更新, サブスクリプション, 支援形態 | `#contract`     |
