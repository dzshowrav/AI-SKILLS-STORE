...
- Prefer plugin-specific stubs already used in the repo.
- If PHPStan still cannot resolve classes, add targeted `ignoreErrors` patterns.
...
- Confirm the baseline file is included and didn't grow unexpectedly.
- Re-run after changing `ignoreErrors`.

## Failure modes / debugging

- "Class not found": confirm autoloading/stubs, or add a narrow ignore pattern
- Huge error counts: reduce `paths`, add `excludePaths`, start at a lower level
- Inconsistent types: add explicit PHPDoc rather than runtime guards
