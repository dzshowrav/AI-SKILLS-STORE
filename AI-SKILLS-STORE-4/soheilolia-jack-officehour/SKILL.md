---
name: jack-officehour
description: >
  Evaluates any existing work artifact against Block's intelligence-native operating
  model (Block 2.0). Use when user shares a doc, deck, prototype, workflow, repo, PRD,
  memo, weekly update, Slack draft, or leadership note and asks to 'pressure-test,'
  'grade against Block 2.0,' 'run through the Block lens,' 'review for office hours
  with the CEO,' 'check strategic alignment,' 'find the gaps,' or types '/jack-officehour'.
  Scores across 7 dimensions (capability leverage, world-model value, system-learning
  value, coordination reduction, intelligence-layer contribution, compounding value,
  human judgment design), interprets score shape, catches alignment theater, identifies
  gaps, and shows exactly how to make the work A+ including elite 0.1% strategic moves.
  Do NOT use for summarization, first-draft creation, brainstorming, career coaching,
  grammar editing, or general feedback unless the user explicitly asks to evaluate
  existing work against Block's strategic direction.
metadata:
