...
**Recon-first, then bake it into the creator's packet (the highest-leverage move).** Before delegating story creation, the leader does a short **scoping recon directly** — open the story's design contract AND the real target repo, and answer: what already exists that the story must NOT rebuild (prior stories often shipped half of it); what's the net-new delta; which design-doc claims drift from the installed code (Rule 31 — verify the loader/path/contract yourself); and any seam ruling the design leaves open — **especially any cross-entity/cross-aggregate write the design describes as happening "inside the fold / on the other entity" (Rule 35): a pure single-entity fold can't do that, so pre-decide the command/orchestration seam now rather than letting the dev guess it (Rule 36).** Then **pass that recon verbatim in the Delegation Packet's _Knowledge sources_ + _Prior findings_ slots, with the rulings pre-decided.** Doing this per story is the single biggest driver of clean first-pass story files: the creator renders the recon into the BMAD format instead of re-deriving it (and often catches a *further* drift the leader missed — which the leader then re-verifies per Rule 31). The recon costs the leader a few greps and reads; it saves a fix round-trip on the spec and a worse one in code. (Don't over-invest for a trivial story — but for anything touching real wiring, recon first.)
...
**Disciplines that repeatedly catch real bugs here (see Critical Rules 23–25, 29–33, and 37–39):**
...
- **Confirm at least one e2e drives the REAL entry point** (the pipeline / command / intake the app actually uses), not just isolated components seeded via repositories (Rule 24) — and that it feeds the REAL delivery shape, not a fabricated one (Rule 37: a test that publishes an event N× or to correlations the real keying won't reach is a silent false-green, usually masking a cross-aggregate-write fork — Rule 35).
- **Don't accept a sub-agent's "pre-existing flake / unrelated" attribution at face value (Rule 38)** — re-run independently and confirm it's the known flake, not a regression your story just introduced; scrutinize hardest when you recently fixed that flake or the story touched code the "unrelated" suite boots. And if a flake recurs across two stories, root-cause it (Rule 39) — a flaky gate erodes the signal that distinguishes real failures from noise.
